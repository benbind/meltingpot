...
def p_mean(tensor, power):
  mean_tensor = tensor.pow(power).mean(0)
  return (mean_tensor * mean_tensor.sign()).pow(1 / power).abs()
...
if __name__ == "__main__":
  ...
  if args.sigma_vals:
    if args.sigma_vals == "1":
      ...
      selfishness = list(
          torch.distributions.Beta(3, 2).sample((num_agents,)).numpy()
      )
    elif args.sigma_vals == "4":
      selfishness = list(
          torch.distributions.Beta(4, 2).sample((num_agents,)).numpy()
      )
    elif args.sigma_vals == "5":
      selfishness = list(
          torch.distributions.Beta(5, 2).sample((num_agents,)).numpy()
      )
    elif args.sigma_vals == "6":
      selfishness = list(
          torch.distributions.Beta(7, 2).sample((num_agents,)).numpy()
      )
    elif args.sigma_vals == "7":
      selfishness = list(
          torch.distributions.Beta(9, 2).sample((num_agents,)).numpy()
      )

  else:
    selfishness = np.random.uniform(size=[num_agents])

  principal.vote_p_value(selfishness)
  ...
  raw_rewards = torch.zeros((args.sampling_horizon, num_envs)).to(device)
  taxed_rewards = torch.zeros((args.sampling_horizon, num_envs)).to(device)
  ...
  #   cumulative_raw_rewards = torch.zeros((args.sampling_horizon, args.num_parallel_games, num_agents)).to(device)
  #   cumulative_taxed_rewards = torch.zeros((args.sampling_horizon, args.num_parallel_games, num_agents)).to(device)
  ...
  num_policy_updates_total = 1500  #!! hard code at 500 for 500 total updates
  num_updates_for_this_ep = 0
  current_episode = 1
  episode_step = 0
  episode_rewards = torch.zeros(num_envs).to(device)
  episode_raw_rewards = torch.zeros(num_envs).to(device)
  episode_taxed_rewards = torch.zeros(num_envs).to(device)
  ...
  for update in range(1, num_policy_updates_total + 1):
    ...
    for step in range(0, args.sampling_horizon):
      ...
      raw_reward = copy.deepcopy(extrinsic_reward)
      ...
      for game_id in range(args.num_parallel_games):
        game_reward = extrinsic_reward[
            game_id * num_agents : (game_id + 1) * num_agents
        ]
      ...
      for env_id in range(len(reward)):
        ...
        reward[env_id] = intrinsic_reward[env_id]
      ...
      raw_rewards[step] = torch.tensor(raw_reward).to(device).view(-1)
      taxed_rewards[step] = torch.tensor(extrinsic_reward).to(device).view(-1)
    ...
    episode_rewards += torch.sum(
        rewards, 0
    )  # tensor([7.1399, 2.7199, 0.3362, 0.3514, 0.3953, 0.3367, 5.7416],
    episode_raw_rewards += torch.sum(raw_rewards, 0)
    episode_taxed_rewards += torch.sum(taxed_rewards, 0)
    ...
    if num_updates_for_this_ep == 5:
      ...
      writer.add_scalar(
          "charts/mean_raw_return", torch.mean(episode_rewards), current_episode
      )

      writer.add_scalar(
          "charts/voted_p_value", principal.p_value, current_episode
      )

      writer.add_scalar(
          "charts/p_mean_raw_return",
          p_mean(episode_raw_rewards, principal.p_value),
          current_episode,
      )

      writer.add_scalar(
          "charts/mean_raw_return",
          torch.mean(episode_raw_rewards),
          current_episode,
      )

      writer.add_scalar(
          "charts/p_mean_taxed_return",
          p_mean(episode_taxed_rewards, principal.p_value),
          current_episode,
      )

      writer.add_scalar(
          "charts/mean_taxed_return",
          torch.mean(episode_taxed_rewards),
          current_episode,
      )
      writer.add_scalar(
      ...
      for game_id in range(args.num_parallel_games):
        # writer.add_scalar(f"charts/principal_return_game{game_id}", principal_episode_rewards[game_id].item(), current_episode)
        tax_rates = tax_values[f"game_{game_id}"]
        for bracket in range(len(tax_rates)):
          writer.add_scalar(
              f"setup/tax_value_{f'game_{game_id}'}_bracket_{bracket+1}",
              tax_rates[bracket],
              0,
          )
